...
const personsAge = persons.reduce((accum, person) => {
  return accum + person.age;
}, 0);

console.log(`La suma total de las edades de las personas es ${personsAge}`);

// //  *  - 2. Obtén la suma total de todas las edades de las personas francesas.

const frenchPeopleAge = persons
  .filter((person) => {
...
console.log(`La suma total de las edades de las personas de Francia es ${frenchPeopleAge}`);

// // 3. Obtén un array con el nombre de todas las mascotas.

const petsArray = persons.map((person) => {
  const copyPerson = { ...person };
  return copyPerson.pet.name;
});
console.log(petsArray);

// - 4. Obtén un array con las personas que tengan gato.

const personsWithCat = persons.filter((person) => {
...
console.log(spaniardsCars);

//  *  - 7. ¡Bonus point! Obtén un array con todas las personas en el que cada persona tenga toda
//  *       la info de su coche. Ejemplo a continuación:
const personWithCarInfo = persons.map((person) => {
  const personCopy = { ...person };
  const car = cars.find((car) => {
    return car.id === personCopy.car;
  });
  personCopy.car = car;
  return personCopy;
});
console.log(personWithCarInfo);

// - 6. Obtén un array con las personas que tengan un coche de la marca Ford.

const personsWithFord = personWithCarInfo.filter((person) => {
